...
export default function Diagnostic() {
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Target className="text-indigo-500" /> Diagnóstico de nivel
          </h1>
        ...
        <Card className="p-6">
          ...
          {user.diagnostic && (
            <div className="mt-4 p-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/30">
              <p className="text-sm">
  ...
  if (finished) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Award className="text-indigo-500" /> Tu diagnóstico
          </h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Nivel general: <span className="font-semibold text-indigo-600">{results.overallLevel}</span>
          </p>
        </div>
        <Card className="p-6">
          <div className="grid sm:grid-cols-2 gap-3">
            {SKILLS.map((sk) => {
              ...
              return (
                <div key={sk} className={`p-3 rounded-xl border ${isWeakest ? "border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-900/20" : isStrongest ? "border-violet-300 dark:border-violet-700 bg-violet-50 dark:bg-violet-900/20" : "border-gray-200 dark:border-gray-700"}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="flex items-center gap-2 font-medium text-sm"><Icon size={16} /> {SKILL_LABELS[sk]}</span>
                    <span className="text-xs font-bold text-indigo-600">{r.level}</span>
                  </div>
          ...
          <div className="mt-5 p-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/30">
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Target className="text-indigo-500" /> Diagnóstico
        </h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
        ...
        <div className="mt-5 space-y-2">
          {q.options.map((option) => (
            <button
              ...
              className="w-full text-left p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition"
            >
              {option}
            </button>
          ))}
